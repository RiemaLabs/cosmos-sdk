...
import (
	...
	//nolint // using just for backwards compat
...
const (
	PrivKeySize = 32
	keyType     = "taproot"
	PrivKeyName = "tendermint/PrivKeyTaproot"
	PubKeyName  = "tendermint/PubKeyTaproot"
)
...
// Tweak the pubkey to be used in the key spend path as defined in BIP-340.
func (privKey *PrivKey) PubKey() cryptotypes.PubKey {
	privKeyObj := secp256k1.PrivKeyFromBytes(privKey.Key)
	pubkeyObject := privKeyObj.PubKey()
	return &PubKey{Key: TweakPubKey(pubkeyObject)}
}
...
func (pubKey *PubKey) Address() crypto.Address {
	...
	// Both the key and the address are a 32-byte x-only public key as specified in BIP-340.
	return pubKey.Key[1:]
}

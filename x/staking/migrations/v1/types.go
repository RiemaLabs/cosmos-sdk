...
func GetValidatorsByPowerIndexKey(validator types.Validator) []byte {
	...
	key := make([]byte, 1+powerBytesLen+v1auth.ValAddrLen)
...
func GetREDKey(delAddr sdk.AccAddress, valSrcAddr, valDstAddr sdk.ValAddress) []byte {
	key := make([]byte, 1+v1auth.ValAddrLen*2+v1auth.AddrLen)

	copy(key[0:v1auth.AddrLen+1], GetREDsKey(delAddr.Bytes()))
	copy(key[v1auth.AddrLen+1:v1auth.AddrLen+v1auth.ValAddrLen+1], valSrcAddr.Bytes())
	copy(key[v1auth.AddrLen+v1auth.ValAddrLen+1:v1auth.AddrLen+2*v1auth.ValAddrLen+1], valDstAddr.Bytes())
...
func GetREDByValSrcIndexKey(delAddr sdk.AccAddress, valSrcAddr, valDstAddr sdk.ValAddress) []byte {
	...
	key := make([]byte, len(REDSFromValsSrcKey)+v1auth.AddrLen+v1auth.ValAddrLen)
	copy(key[0:offset], REDSFromValsSrcKey)
	copy(key[offset:offset+v1auth.AddrLen], delAddr.Bytes())
	copy(key[offset+v1auth.AddrLen:offset+v1auth.AddrLen+v1auth.ValAddrLen], valDstAddr.Bytes())
...
func GetREDByValDstIndexKey(delAddr sdk.AccAddress, valSrcAddr, valDstAddr sdk.ValAddress) []byte {
	...
	key := make([]byte, len(REDSToValsDstKey)+v1auth.AddrLen+v1auth.ValAddrLen)
	copy(key[0:offset], REDSToValsDstKey)
	copy(key[offset:offset+v1auth.AddrLen], delAddr.Bytes())
	copy(key[offset+v1auth.AddrLen:offset+v1auth.AddrLen+v1auth.ValAddrLen], valSrcAddr.Bytes())
...
func GetREDKeyFromValSrcIndexKey(indexKey []byte) []byte {
	...
	kv.AssertKeyLength(indexKey, v1auth.AddrLen+2*v1auth.ValAddrLen+1)

	valSrcAddr := indexKey[1 : v1auth.ValAddrLen+1]
	delAddr := indexKey[v1auth.ValAddrLen+1 : v1auth.ValAddrLen+1+v1auth.AddrLen]
	valDstAddr := indexKey[v1auth.AddrLen+v1auth.ValAddrLen+1 : v1auth.AddrLen+2*v1auth.ValAddrLen+1]
...
func GetREDKeyFromValDstIndexKey(indexKey []byte) []byte {
	// note that first byte is prefix byte
	kv.AssertKeyLength(indexKey, 2*v1auth.ValAddrLen+1)

	valDstAddr := indexKey[1 : v1auth.ValAddrLen+1]
	delAddr := indexKey[v1auth.ValAddrLen+1 : v1auth.AddrLen+v1auth.ValAddrLen+1]
	valSrcAddr := indexKey[v1auth.AddrLen+v1auth.ValAddrLen+1 : v1auth.AddrLen+2*v1auth.ValAddrLen+1]
...
func GetUBDKeyFromValIndexKey(indexKey []byte) []byte {
	...
	kv.AssertKeyLength(addrs, v1auth.ValAddrLen+v1auth.AddrLen)

	kv.AssertKeyAtLeastLength(addrs, v1auth.ValAddrLen+1)
	valAddr := addrs[:v1auth.ValAddrLen]
	delAddr := addrs[v1auth.ValAddrLen:]

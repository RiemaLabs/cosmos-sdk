...
func (suite *AddressSuite) TestModule() {
	...
	expected := Hash("module", []byte(modName))

	assert.Equal(expected, addrLegacy, "when no derivation keys, we fall back to the legacy module address using sha256 of the module name")
